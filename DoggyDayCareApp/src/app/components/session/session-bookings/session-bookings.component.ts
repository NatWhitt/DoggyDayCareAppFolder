...
export class SessionBookingsComponent {
  ...
  ngOnInit(): void {
    this.bookingService
      .getBookingsSingleSession(this.session.id ?? 0)
      .subscribe((booking) => (this.bookedDogs = booking));
    this.bookingService
      .getAllAvailableDogsForSingleSession(this.session.id ?? 0)
      .subscribe((dogs) => (this.avilableDogs = dogs));
  }
  ...
  deleteBooking(bookedDog: Dog) {
    ...
    this.bookingService
      ...
      .subscribe(
        () =>
          (this.bookedDogs = this.bookedDogs.filter(
            (d) => d.id !== bookedDog.id
          ))
      );
  }
